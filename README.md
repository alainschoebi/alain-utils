# alain-utils
Various Python utilities including loggers, SE(3) poses, colors, 2D bounding boxes, function decorators, Kalman filtering, Plotly visualization functions, and more.
...
Some specialized utilities in `alain-utils` rely on additional packages and are only available if the corresponding packages are installed. Optional dependencies include `plotly`, `opencv`, `tqdm`, `shapely`, `pycolmap`, `cython-bbox`, `imageio[ffmpeg]`, and ROS.
